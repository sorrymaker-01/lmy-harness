...
func TestPersistentStoreRestoresConversationMessagesAndMemory(t *testing.T) {
	...
	if memory.Summary == "" || memory.Summary == "No prior short-term memory." {
...
func TestPersistentStoreDeleteConversation(t *testing.T) {
	...
	if memory := restored.GetShortMemory(conversation.ID); memory.Summary != "No prior short-term memory." {
